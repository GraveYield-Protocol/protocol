...
pub struct SalvagePool<'info> {
    ...
    /// LP-holder share vault — native-SOL system account, system-owned, 0-data.
    /// Created on first salvage of this pool via `init_if_needed`; subsequent
    /// (would-be) salvages of the same pool are blocked at the `pool_registry`
    /// init gate, so init_if_needed only matters on the first call. Charter
    /// invariant: this account is UNSWEEPABLE by any admin key, ever; only
    /// `claim_lp_proceeds` may debit it against a valid Merkle proof.
    #[account(
        init_if_needed,
        payer = salvor,
        space = 0,
